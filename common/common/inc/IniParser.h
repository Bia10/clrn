   ...
   bool INI<T, U, V>::Parse()
{
   ...
   std::copy(std::istream_iterator<string_t>(file), std::istream_iterator<string_t>(), std::back_inserter(strings));

   std::vector<string_t>::const_iterator it = strings.begin();
   const std::vector<string_t>::const_iterator itEnd = strings.end();

   for (; it != itEnd; ++it)
   {
      char_t* line = const_cast<char_t*>(it->c_str());
      Nake(line);
